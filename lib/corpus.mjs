...
export async function getTaamimFrequencies(corpus = 'torah', includeNach = false) {
  const system = resolveCorpus(corpus);
  const cacheKey = `${system}:${includeNach ? 'nach' : 'core'}`;
  ...
  for (const loaded of sources) {
    for (const verse of loaded.indexedVerses) {
      ...
      const sequence = verse.verseTaamimSequence;
      if (!sequence) {
        continue;
      }
  ...
  frequencyCache.set(cacheKey, payload);
  return payload;
}
